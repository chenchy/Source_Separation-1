...
for i in range(50):
	...
	for file in os.listdir('./tmp_ori/'):
		if file.split('_')[2].split('.')[0] == str(i):
			if file.split('_')[0] == '0':
				vocals.append(np.load('./tmp_ori/'+file))
			if file.split('_')[0] == '1':
				drums.append(np.load('./tmp_ori/'+file))
			if file.split('_')[0] == '2':
				bass.append(np.load('./tmp_ori/'+file))
			if file.split('_')[0] == '3':
				others.append(np.load('./tmp_ori/'+file))
	...
	plt.savefig(f'pic/ori/{str(i)}.png')
